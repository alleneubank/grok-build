...
pub struct PermissionRequestHookContext {
    ...
    /// Same snapshot session dispatch uses, so a user-disabled hook is skipped here too.
    pub disabled: Arc<xai_grok_hooks::trust::DisabledHooks>,
}
...
pub async fn fire_permission_request(
    ...
    if !ctx
        .registry
        .has_enabled_hooks_for_canonical(HookEventName::PermissionRequest, &ctx.disabled)
    {
        return;
    }
    ...
    let run_ctx = RunContext {
        ...
        process_scope: None,
        disabled: Arc::clone(&ctx.disabled),
    };
